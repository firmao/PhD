\textit{In this chapter, we discuss how linking entries across heterogeneous data sources such as databases or knowledge bases, becomes an increasingly difficult problem, in particular w.r.t. the runtime of these tasks. 
...
In \Cref{mfkc}, we present our nested filters that allow reducing the runtime of the MFKC similarity joins. We then evaluate our approach in \Cref{evaluation}, where we focus on approaches that aim to improve the time-efficiency of the link discovery task. 
...
Such function works with three parameters, i.e. two strings $s$ and $t$ and an integer $lim$ and returns the sum of frequencies, where $\f{c_i, s}$ is a function that returns the frequency of the character $c_i$ in the string $s$ and $s \supseteq \{c_1,...,c_n\}$, i.e $\f{a,"andrea"}=2$, because the character $a$ has been found twice and the hash functions $h(s)$ and $h(t)$ containing the characters and their frequencies. The output of function is always positive, as shown in \Cref{eq:naive}. 
...
\Cref{eq:1} shows our definition for the string similarity function $\upsigma$ for the MFKC.
...
In this section, the runtime of MFKC defined in \Cref{eq:1} is improved using filters where $\mathcal{N}$ is the output of first frequency filter, $\mathcal{L}$ is the output of hash intersection filter and $\mathcal{A}$ represents the output of the $k$ similarity filter. 
...
\begin{proof}[\Cref{thr:ff1}]
	Let the intersection between hashes $h(t,k)$ and $h(s,k)$ be a set of characters from $c_1$ to $c_n$, such that \Cref{eq:intersection}: 
	\begin{equation} \label{eq:intersection}
	h(t,k) \cap h(s,k)=\{c_1,...,c_n\}
	\end{equation}
	According to the definition of the frequencies $\f(c_i, t)$ we have \Cref{eq:tsub}: 
	...
	Therefore, from \Cref{eq:proof1} and \Cref{eq:proof2}, we obtain the \Cref{eq:obt1}:
...
we also can say that the \Cref{h:1} represents a valid implication.
\begin{equation} \label{h:1}
	h(s) \cap h(t) = \emptyset \Rightarrow \upsigma(s,t)=0
\end{equation}

The \Cref{h:1} means that if the intersection between $h(s,k)$ and $h(t,k)$ is a empty set, this implies that the similarity score will be 0.
...
This filter provides a validation and we show that the score of previous $k$ similarity is always lower than the next $k$, according to the \Cref{eq:m1} and in some cases when the similarity score is been reached before compute all elements $\in \h{s,k} \cap \h{t,k}$, thus saving computation in these cases. 
...
We rewrote the equation for the first iteration, according to \Cref{eq:m2}
...
Let $k \in \mathbb{Z}_+$ be given and suppose the equation \Cref{eq:m1} is true for $k+1$. %Then (see \Cref{eq:k1,eq:k2,eq:k3})
...
Therefore, we can notice that the sum of frequencies will be always greater or equal 0, according to $\f{c_{k+1},s}+\f{c_{k+1},t} \geq 0$. Thus, \Cref{eq:m1} holds true.
...
\begin{proof}[\Cref{thr:cc}]
	...
	\end{equation}
    which is the definition of $\mathcal{A}^*$ in \Cref{eq:aa1}. Therefore, $\mathcal{A}=\mathcal{A}^*$.
\end{proof}
%
...
An example to show the efficiency of each filter can be found at \Cref{fig:avoided2,fig:avoided}, where 10,273,950 comparisons from DBpedia+LinkedGeoData were performed and Performance Gain $(PG)=Recall(\mathcal{N}) + Recall(\mathcal{L})$. 
The recall can be seen in \Cref{fig:recall}.
...
The parallel implementation works better in large datasets with size more than $10^5$, that was more than one time faster than the approach without parallelism and two times faster than the naive approach as shown in \Cref{fig:a,fig:b,fig:c}.

\subsection{Runtime Evaluation}

% We evaluate the runtime of all filter combinations with 1,001,642 and 10,273,950 pairs of labels from DBpedia and LinkedGeodata, as can be observed in \Cref{fig:a,fig:b}.
The evaluation in \Cref{fig:a,fig:b} shows that all filter setup outperform the naive approach, and the parallel approach does not suffer significant changes related to the runtime according to the size of the dataset, as show \Cref{fig:c}.
...
The experiments related to the variance of $k$, also were considered, as show in \Cref{fig:d}, the runtime varies according the size of $k$, indicating the influence of $k$ with values from $1$ to $120$ with $1,001,642$ comparisons.
% \begin{table}[H]
...
% 	\label{tab.runtime}
% \end{table}
The performance (run-time) was improved as shown in \Cref{fig:a,fig:b} and according the recall with a performance gain of $26.07 \%$ as shown in \Cref{tab.avoided}. The time complexity is based on two sort process O(n log n) + O(m log m) resulting in O(n log n) as a upper bound in the worst case.
...
In the experiments (see \Cref{fig:c,fig:e,fig:f}), we looked at the growth of the runtime of our approach on datasets of growing sizes.
...
We also can realize the quantity of avoided pairs in each combination of filters in \Cref{fig:avoided2}, that consequently brings a performance gain.
We looked at experiments with runtime behavior on a large dataset with more than $10^6$ labels as shown in \Cref{fig:b}. The results suggest that the runtime decreases according to the threshold $\theta$ increment. Thus, one more point showing that our approach is useful on large datasets, where can be used with high threshold values for link discovery area.
About our parallel implementation, \Cref{fig:c} shows that our GPU parallel implementation works better on large datasets with size greater than $10^5$. 
%Also we have the CPU experiments as shown in \Cref{fig:e,fig:f}.
...
As shown in \Cref{fig:c,fig:e,fig:f}, our approach outperforms not only the naive approach, but also Jaccard Index. We show that the threshold $\theta$ and $k$ have a significant influence related to the runtime.
...
Our work fix all these problems and also has a better runtime, as show \Cref{fig:a}, \Cref{fig:b} and \Cref{fig:c}.

%In order to summarize main differences between the naive and our approach, also  you can see the \Cref{tab:diff}.
