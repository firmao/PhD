...
\end{itemize} 
%
The remainder of this chapter is structured as follows: \Cref{approach} presents our algorithm for the detection of erroneous links in large-scale link repositories; \Cref{sessionErrorQuality} presents the error types and a quality measure for linksets; \Cref{evaluation} presents the evaluation of our approach.
...
In \Cref{fig:ErrorDetection}, we show how our algorithm works. Given datasets $D_1,...,D_n$, resources $R=\{a,b,c,d,e,f\}$, the idea is to detect two or more resources sharing the same dataset inside the same cluster, following the steps described in the following list and into the \cref{alg:err}.
...
%Using the symbols from \cref{tab:symbols} we will formalize the problem. 
...
%We identified two types of errors, in which can be defined in quality dimensions by~\cite{zaveri2015quality}. (1) \textbf{Semantic Accuracy} errors, in which we detect if data values correctly represent the real-world facts. (2) \textbf{Consistency} errors where a knowledge base is free of (logical or formal) contradictions concerning particular knowledge representation and inference mechanisms. The \cref{fig:errorType} shows a fictional example of both of error types, in which we represent links between  GeoNames\footnote{\url{http://www.geonames.org/}} and DBpedia\footnote{\url{http://dbpedia.org/}}.
\begin{figure}[H]
	\centering
...
\begin{figure*}[htb]
	...
	\caption{Error rank (legends: see \cref{tab:legends}) and 
...
The experiments were performed with the input size varying between $10^3$ and $10^6$ RDF triples using the configuration (1), as shown in \Cref{fig:r2,fig:r3}. Our algorithm processed all $19,200,114$ links from LinkLion in $4.6$ minutes with the configuration (2). 
...
To illustrate this part of our idea, we can state: Given a graph $G(V,E)$, that contains all linksets from the repository $G(V,E) \subseteq \mathcal{L}$, this graph has partitions $P \subset G(V,E)$. Thus, errors are calculated for each partition, the processes are separated in threads and these threads are spread among CPU/GPU cores. Thus, we process the graph partitions in parallel, as shown in \Cref{fig:cpugpu1}.
%, according to the parallel processing as shown in \cref{fig:cpugpu2}.
...
\Cref{fig:reasoners} shows that CEDAL is significantly faster than Pellet, reaching up to three orders of magnitude of speedup when faced with $10^6$ triples.
...
%
%As show in \cref{fig:reasoners}, we can say that for our aim that is calculate the RDF graph partitions CEDAL outperform Pellet. 
%
%\begin{figure}[] 
%	\begin{tikzpicture}
%	\begin{axis}[
